...
class XmlHttpBodyMatchChecker implements \PhpPact\Matchers\Checkers\IMatchChecker
{
     ...
     * @return array
     */
    function findEligibleNodesByJsonPath($xmlStr, $jsonPath) {
        $eligibleNodes = array();
     ...
     */
    private function extractNode($domDocument, $token = false, $enumeration, $maxDepth) {
        $baseNode = false;
        if ($token === false) {
            $i = 0;
        ...
        } else if (is_numeric($enumeration)) {
